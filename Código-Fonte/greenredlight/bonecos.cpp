...
template <>
struct hash<Vertex6> {
  size_t operator()(Vertex6 const& vertex) const noexcept {
...
void Bonecos::createBuffers(GLuint program) {
  m_program = program;
...
void Bonecos::loadObj(std::string_view path, bool standardize) {
  ...
  std::unordered_map<Vertex6, GLuint> hash{};
...
void Bonecos::render(Camera m_camera) {
  ...
  glm::mat4 model{1.0f};
  model = glm::translate(model, glm::vec3(-1.75f, 0.33f, 4.0f)); 
  ...
  model = glm::mat4(1.0);
  model = glm::translate(model, glm::vec3(0.75f, 0.33f, 3.5f));
  model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 1, 0)); 
  model = glm::scale(model, glm::vec3(0.5f)); 

  m_camera.reinitModel(m_program, model);
  glDrawElements(GL_TRIANGLES, m_indices.size(), GL_UNSIGNED_INT, nullptr);

  model = glm::mat4(1.0);
  model = glm::translate(model, glm::vec3(-1.0f, 0.33f, 2.0f)); 
  model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 1, 0)); 
  model = glm::scale(model, glm::vec3(0.5f));
  ...
  model = glm::mat4(1.0);
  model = glm::translate(model, glm::vec3(1.75f, 0.33f, 2.25f)); 
  model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 1, 0)); 
  model = glm::scale(model, glm::vec3(0.5f)); 
  ...
  model = glm::mat4(1.0);
  model = glm::translate(model, glm::vec3(0.55f, 0.33f, 1.60f));
  model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 1, 0)); 
  model = glm::scale(model, glm::vec3(0.5f)); 
  ...
  model = glm::mat4(1.0);
  model = glm::translate(model, glm::vec3(1.6f, 0.33f, 0.5f));
  model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 1, 0)); 
  model = glm::scale(model, glm::vec3(0.5f)); 
  ...
  model = glm::mat4(1.0);
  model = glm::translate(model, glm::vec3(-1.85f, 0.33f, 0.6f));
  model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 1, 0)); 
  model = glm::scale(model, glm::vec3(0.5f));
...
void Bonecos::computeNormals() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    // Get face vertices
    Vertex6& a{m_vertices.at(m_indices.at(offset + 0))};
    Vertex6& b{m_vertices.at(m_indices.at(offset + 1))};
    Vertex6& c{m_vertices.at(m_indices.at(offset + 2))};
...
void Bonecos::computeTangents() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    ...
    // Get face vertices
    Vertex6& v1{m_vertices.at(i1)};
    Vertex6& v2{m_vertices.at(i2)};
    Vertex6& v3{m_vertices.at(i3)};
...
void Bonecos::loadDiffuseTexture(std::string_view path) {
...
void Bonecos::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path)) return;
