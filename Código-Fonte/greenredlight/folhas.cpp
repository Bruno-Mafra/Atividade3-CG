...
template <>
struct hash<Vertex7> {
  size_t operator()(Vertex7 const& vertex) const noexcept {
...
void Folhas::createBuffers(GLuint program) {
  m_program = program;
...
void Folhas::loadObj(std::string_view path, bool standardize) {
 auto basePath{std::filesystem::path{path}.parent_path().string() + "/"};
  ...
  std::unordered_map<Vertex7, GLuint> hash{};
...
void Folhas::render(Camera m_camera) {
...
void Folhas::computeNormals() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    // Get face vertices
    Vertex7& a{m_vertices.at(m_indices.at(offset + 0))};
    Vertex7& b{m_vertices.at(m_indices.at(offset + 1))};
    Vertex7& c{m_vertices.at(m_indices.at(offset + 2))};
...
void Folhas::computeTangents() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    ...
    // Get face vertices
    Vertex7& v1{m_vertices.at(i1)};
    Vertex7& v2{m_vertices.at(i2)};
    Vertex7& v3{m_vertices.at(i3)};
...
void Folhas::loadDiffuseTexture(std::string_view path) {
...
void Folhas::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path)) return;
