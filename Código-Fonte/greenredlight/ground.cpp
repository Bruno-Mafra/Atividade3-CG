...
void Ground::createBuffers(GLuint program) {
  m_program = program;
...
void Ground::render(Camera m_camera) {
  ...
  for (const auto z : iter::range(-N, N + 1)) {
    for (const auto x : iter::range(-N, N + 1)) {
      model = glm::mat4(1.0f);
      model = glm::translate(model, glm::vec3(1.5f*x, 0.0f, 1.25f*z));
      model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0, 0, 1)); //girei 180 pra ver se mudava a luz
...
void Ground::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path)) return;
