...
void Paredes::createBuffers(GLuint program) {
  m_program = program;
...
void Paredes::loadObj(std::string_view path, bool standardize) {
  ...
  std::unordered_map<Vertex2, GLuint> hash{};
...
void Paredes::render(Camera m_camera) {
...
void Paredes::computeNormals() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    // Get face vertices
    Vertex2& a{m_vertices.at(m_indices.at(offset + 0))};
    Vertex2& b{m_vertices.at(m_indices.at(offset + 1))};
    Vertex2& c{m_vertices.at(m_indices.at(offset + 2))};
...
void Paredes::computeTangents() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    ...
    // Get face vertices
    Vertex2& v1{m_vertices.at(i1)};
    Vertex2& v2{m_vertices.at(i2)};
    Vertex2& v3{m_vertices.at(i3)};
...
void Paredes::loadDiffuseTexture(std::string_view path) {
...
void Paredes::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path)) return;
