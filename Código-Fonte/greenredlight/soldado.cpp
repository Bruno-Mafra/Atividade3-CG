...
void Soldado::render(Camera m_camera) {
  ...
  abcg::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
...
void Soldado::computeNormals() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    // Get face vertices
    Vertex4& a{m_vertices.at(m_indices.at(offset + 0))};
    Vertex4& b{m_vertices.at(m_indices.at(offset + 1))};
    Vertex4& c{m_vertices.at(m_indices.at(offset + 2))};
...
void Soldado::computeTangents() {
  ...
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    ...
    // Get face vertices
    Vertex4& v1{m_vertices.at(i1)};
    Vertex4& v2{m_vertices.at(i2)};
    Vertex4& v3{m_vertices.at(i3)};
...
void Soldado::loadDiffuseTexture(std::string_view path) {
...
void Soldado::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path)) return;
